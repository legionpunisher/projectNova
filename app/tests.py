...
class ProfileTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(id =1, username='major')
        self.profile = Profile.objects.create(user = self.user,bio = 'blow away',email='d54s@test.com')
...
class BusinessTest(TestCase):
    def setUp(self):
        self.mpesa= Business.objects.create(b_name='star',b_description='dope',b_email='gvftf@test.com')
...
class PostsTest(TestCase):
    def setUp(self):
        ...
        self.security= Posts.objects.create(title='life',content='future',posted_by= self.user, hood= self.south)
